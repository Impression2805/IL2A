...
class iCIFAR100(CIFAR100):
    ...
    def getTrainData_icarl(self, classes, exemplar_set):
        datas, labels = [], []
...
class iSVHN(SVHN):
    def __init__(self, root,
                 ...
                 target_test_transform=None,
                 download=False):
        super(iSVHN, self).__init__(root,
                                       split=train,
                                       transform=transform,
                                       target_transform=target_transform,
                                       download=download)
    ...
    def getTestData(self, classes):
        datas, labels = [], []
        for label in range(classes[0], classes[1]):
            data = self.data[np.array(self.labels) == label]
            datas.append(data)
            labels.append(np.full((data.shape[0]), label))
        datas, labels = self.concatenate(datas, labels)
        self.TestData = datas if self.TestData == [] else np.concatenate((self.TestData, datas), axis=0)
    ...
    def getTrainItem(self,index):
        img = np.transpose(self.TrainData[index], (1, 2, 0))
        img, target = Image.fromarray(np.uint8(img)), self.TrainLabels[index]
    ...
    def getTestItem(self,index):
        img = np.transpose(self.TestData[index], (1, 2, 0))
        img, target = Image.fromarray(np.uint8(img)), self.TestLabels[index]
        if self.test_transform:
            img=self.test_transform(img)
